...
@OnlyIn(Dist.CLIENT)
public class WoodGolemModel extends HierarchicalModel<WoodGolem> {
    ...
    public WoodGolemModel(ModelPart modelPart) {
        this.root = modelPart;
    ...
    @Override
    public void setupAnim(@NotNull WoodGolem woodGolem, float legXRot, float legYRot, float p_102621_, float headYRot, float headXRot) {
        this.head.yRot = headYRot * ((float)Math.PI / 180F);
        this.head.xRot = headXRot * ((float)Math.PI / 180F);
        this.rightLeg.xRot = -1.5F * Mth.triangleWave(legXRot, 13.0F) * legYRot;
        this.leftLeg.xRot = 1.5F * Mth.triangleWave(legXRot, 13.0F) * legYRot;
